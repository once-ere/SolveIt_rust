...
line-faithful translation of SUNDIALS 7.7.0 --- itself zero-unsafe and
...
\code{boundary} & both & RigidBody had \code{Box<dyn Boundary>} (SDF trait); RigidBody3D an enum. The \textbf{enum} keeps the name --- \textbf{six variants} now: the legacy \code{Point}/\code{Sphere}/\code{Cuboid} plus \code{Torus\{ring\_radius, tube\_radius\}}, \code{Disk\{radius\}} and \code{Cylinder\{radius, half\_height\}}, each with an exact SDF; the SDF behavior survives as the \code{Sdf} trait (verbatim central-difference normal) implemented \emph{for} the enum --- no trait objects, struct stays \code{Clone}\\
\end{longtable}
...
cuboid $\tfrac m3(h^2{+}h^2)$ diagonals; Point $\to$ zero; the three
shapes added by the TORUS/DISK/CYLINDER release get the closed forms
below).
...
--- the farthest reach along a direction (for the torus, of its convex
hull: a support function cannot see the hole) ---
...
magics; \code{--script} replays files; \code{--machine} speaks
...
\section{Thirteen more worked examples}
...
Out[15]= t = 0.1 (2259 solver steps, 100 snapshots, |dE/E| = 1.031e-9,
         132 collision(s) --- CONTACTS lists them)
In[16]:= energy
...
produces \textbf{132 collisions} spanning all three dispatch tiers of
\S\ref{sec:collisions}, and total energy survives every one of them to
$|\Delta E/E| = 1.031\times10^{-9}$. Momentum does \textbf{not}
survive --- $(100, 200, 100) \to (243.59, -82.35, -4.55)$ --- and that
...
evaluation, eleven worked examples with captured output) live in
...
\textbf{The shape tiers.} With the \code{Boundary} enum at six
variants (\S\ref{sec:union}; created with
\code{NEW TORUS|DISK|CYLINDER \{ ... \}} --- \code{grammar.pdf} has the
parameter grammar), \code{collide.rs} dispatches every pair in three

...
cuboids, tori, disks, cylinders), a ground grid, the world axes,
motion trails, and object labels. If the notebook created a rigid
...
\end{lstlisting}
Sent on connect, on \kw{scene refresh}/\kw{redraw}, and after
hide/show. Geometry travels \textbf{once} (the Rapier idea). Note
...
\code{radius}, a \code{cuboid} its \code{half\_extents}, and --- new
in the shapes-and-box release --- a \code{torus} its
\code{ring\_radius} and \code{tube\_radius}, a \code{disk} its
\code{radius}, a \code{cylinder} its \code{radius} and
\code{half\_height}:
